...
def compute_ilocal(latent_space: np.array,  tuning_curve_model, 
                   nx:int, dx, ny=101, ngamma=20,
    ...
    for igamma in tqdm(range(len(gamma)), 'Computing ilocal'):
        ...
        # p(y|x) - to go back to space of 'latent_space' of shape (nx**2, 10201)
        ...
        # estimates
        logpx = -0.5*sum(latent_space.T * solve(sigma, latent_space.T)) - 0.5*slogdet(2*pi*sigma)[1] # of shape (nx**2,1) # Ok!
