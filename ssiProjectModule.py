...
#         print('s',np.squeeze(tcs))
    ...
    for i,el in enumerate(tcs):
        new_log_likelihood__, new_likelihood__ = POISSON(el) # dim 48 comes from here
...
# ssi with adaptive monte-carlo

def ssi_adaptive_monte_carlo(tcs_input: np.array, num_samples:int=1000, 
                             max_num_samples:int=10**6, error:float=1):
    ''' Calculates SSI using chunking (batch) mode for memory efficiency

    Note: this avoids computing full 48^N neural response space
    with [0, ...48] being the set of possible spike counts

    Args:
        tcs_input (np.array): tuning curves 2D array of shape 
        ... (n_neurons, n_directions)
        num_samples (int): number of monte carlo sampled neural responses
        max_num_samples (int): max number of samples (number of samples
        ... double at each iteration)
        error: error in percent
    
    Returns:
        ssi (np.array): 1D array of shape (N_stimulus)
    '''
    # set monte carlo parameters
    err = 1e4 # starting error

    # format tuning curves
    if tcs_input.ndim==1:
        tcs = np.atleast_2d(tcs_input)
    else:
        tcs = np.copy(tcs_input)
    ...
    # samples random neural responses and estimates 
    # SSI by computing posteriors. It doubles the number 
    # of samples each iteration until convergence.
    # minimize error between old and new ssi
    # calculation until convergence        
    while (num_samples < max_num_samples and err > error):

        # samples "num_samples" random spike counts from Poisson
        # distribution for each neuron and stimulus
        # of shape (n_iter, n_neurons, n_stimuli)
        sample_r = np.random.poisson(tcs, size = (num_samples, *tcs.shape))

        # multiply likelihoods across all neurons (assumes independence)
        # p(r1, r2, ..., rN | stimulus) = p(r1|stim) * p(r2|stim) * ... * p(rN|stim)
        # of shape (n_iter, n_stimuli, n_stimuli)
        post_aux = np.maximum(1e-40, \
                            np.prod(np.maximum(poi.pmf(np.tile(sample_r[:,:,:,None], (1,1,1,stim_len)), \
                            np.tile(tcs[None,:,None, :], (num_samples,1, stim_len,1))), 1e-100), axis = 1))
        
        # compute posterior of shape (n_iter, n_stimuli, n_stimuli)
        # by normalizing
        posterior = post_aux / np.tile(np.sum(post_aux, axis = -1)[:,:,None], (1,1,stim_len))

        # calculate ssi per sampled responses
        # I_sp = log(N_stimuli) - H(s|r) 
        # = log(N_stimuli) + sum( p(s|r) * log(p(s|r)) )
        # of shape (iterations, n_stimuli)
        i_sp = np.log(stim_len) + np.sum(posterior * np.maximum(np.log(posterior),-50), axis = 2)

        # average to get ssi per stimulus 
        # of shape (n_stimuli,) 
        ssi = np.mean(i_sp, axis = 0)        
        ...
        # calculate RMSE to check convergence
        err = np.sqrt(np.mean((ssi - previous_ssi)**2))/np.mean(ssi) * 100
        previous_ssi = np.copy(ssi)

        # double sample size
        # at each iteration
        # allows quick convergence check with small samples first
        num_samples *= 2
...
# ssi with iterative mode


def big_ssi_iterative(tcs):
    ...
    # of shape (48, 48, n_stimulus)
    multidim_likelihood = compute_multidim_likelihood_iterative(likelihoods).squeeze(1)
...
def compute_multidim_likelihood(likelihoods):
    """Compute the product of likelihoods with proper 
    broadcasting for any number of dimensions'''

    Args:
        likelihoods (list(np.marray)): of shape (n_neurons, n_possible_spike_count, ..., n_stimulus)

    Returns:
        _type_: multidimensional likelihood
    """
    n = len(likelihoods)
...
def compute_multidim_likelihood_iterative(likelihoods):
    """

    Args:
        likelihoods (list(np.marray)): of shape (n_neurons, 48, 48, n_stimulus)

    Returns:
        _type_: _description_
    """
    '''Most memory-efficient approach for large N - builds up product iteratively'''
    n = len(likelihoods)
    n_theta = likelihoods[0].shape[-1]
    
    # Start with the first likelihood
    result = likelihoods[0][:, np.newaxis] if n > 1 else likelihoods[0]
    
    # Iteratively multiply with remaining likelihoods
    for i in range(1, n):
        # Add new axis at position i for broadcasting
        new_shape = list(result.shape[:-1]) + [1, n_theta]
        result = result.reshape(new_shape[:-1] + [result.shape[-1]])
        
        # Reshape current likelihood for broadcasting
        likelihood_shape = [1] * (i + 1) + [n_theta]
        likelihood_shape[i] = likelihoods[i].shape[0]
        
        # Multiply
        result = result * likelihoods[i].reshape(likelihood_shape)
    
    return result


def mc_ssi_iterative(tcs_input):         #dim tcs = 4x36
    ''' Calculates SSI by Monte Carlo method. \n Up to 2023-11-12 gives result in nats, from 2023-11-12 (included) gives result in bits'''
    ...
    previous_ssi = np.zeros(stim_len)
    if np.max(tcs_input)<20:
        return big_ssi_iterative(tcs_input)*np.log2(math.e)
    ...
    if K>=10000 and err>epsilon and np.max(tcs_input)<30:
        ssi = big_ssi_iterative(tcs_input)
    return ssi*np.log2(math.e)
...
def get_chunk_ranges(n, chunk_size):
    '''Generate list of (start, end) tuples for chunking'''
    ranges = []
...
def iterate_chunk_combinations(chunk_ranges):
    '''Generate all combinations of chunk indices'''
    from itertools import product
    return product(*chunk_ranges)


def big_ssi_chunked(tuning_curves, n_theta, chunk_size):
    ...
    t0 = time()
    for i,el in enumerate(tuning_curves):

        # Likelihood matrix of shape (n_responses, n_theta)
        # - n_responses is the number of possible spike counts the neuron 
        # could produce (e.g., 0, 1, 2, ..., 47 spikes)
        _, new_likelihood__ = POISSON(el) # dim 48 comes from here
        likelihoods.append(new_likelihood__)
    print("loop:", np.round(time()-t0, 2), "sec")

    # get neuron and response counts
    n_neurons = len(likelihoods)
    n_responses = [llh[0].shape[0] for llh in likelihoods]
    
    # initialize accumulator for SSI
    ssi_accumulator = np.zeros(n_theta, dtype=np.float64)
    
    # generate all chunk indices
    chunk_ranges = [get_chunk_ranges(n_resp, chunk_size) for n_resp in n_responses]

    # process each chunk combination
    total_chunks = np.prod([len(ranges) for ranges in chunk_ranges])
    processed = 0
    
    # loop over chunks
    for chunk_indices in iterate_chunk_combinations(chunk_ranges):
        
        # extract chunk from each likelihood (over response count?)
        likelihood_chunks = []
        for i, (start, end) in enumerate(chunk_indices):
            likelihood_chunks.append(likelihoods[i][start:end,:])
        
        # compute multidimensional likelihood for this chunk
        # likelihood_chunks of shape (n_neurons, chunk_size)
        t0 = time()
        chunk_likelihood = compute_multidim_likelihood(likelihood_chunks)
        print("compute llh:", np.round(time()-t0, 2), "sec")
        return chunk_likelihood
        
        # compute normalization for chunk
        chunk_norm = np.sum(chunk_likelihood, axis=-1)
        chunk_norm[chunk_norm == 0.] = 1e-20
        
        # compute likelihood entropy for chunk
        chunk_lik_entropy = -1. * np.sum(
            chunk_likelihood * np.log(np.maximum(chunk_likelihood, 1e-10)), 
            axis=-1
        )
        
        # compute multidimensional entropy for chunk
        chunk_entropy = (chunk_lik_entropy / chunk_norm) + np.log(chunk_norm)
        
        # accumulate contribution to SSI
        sum_axes = tuple(range(n_neurons))
        ssi_contribution = np.sum(
            chunk_likelihood * np.expand_dims(chunk_entropy, axis=-1), 
            axis=sum_axes
        )
        
        ssi_accumulator += ssi_contribution
    ...
    # final SSI computation
    ssi_theta = np.log(n_theta) - ssi_accumulator
    
    return np.squeeze(ssi_theta)


def mc_ssi_chunked(tcs_input: np.array, chunk_size=10):         #dim tcs = 4x36
    ''' Calculates SSI using chunking (batch) mode for memory efficiency

    Args:
        tcs_input (np.array): tuning curves 2D array of shape 
        ... (n_neurons, n_directions)
        chunk_size (int): size of each chunk over possible spike counts 
        ... count responses (e.g., 10 means we take [0,...10],[11,...20] etc.. 
        ... from [0,...,47])
    '''
    # set monte carlo parameters
    K = 100
    ...
    # number of direction stimulus
    n_theta = len(tcs_input[0])
    ...
    # compute ssi in chunking (batch) mode
    # of possible response spike counts
    if np.max(tcs_input) < 20:
        print("Using batch-mode ...")
        return big_ssi_chunked(tcs_input, n_theta, chunk_size) * np.log2(math.e)
    else:
        ...
        # continue computing ssi in chunking (batch) mode
        # over possible response spike counts
        if K >= 10000 and err > epsilon and np.max(tcs_input) < 30:
            ssi = big_ssi_chunked(tcs_input, n_theta, chunk_size)
    return ssi*np.log2(math.e)
